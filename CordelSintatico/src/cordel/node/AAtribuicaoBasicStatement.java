/* This file was generated by SableCC (http://www.sablecc.org/). */

package cordel.node;

import cordel.analysis.*;

@SuppressWarnings("nls")
public final class AAtribuicaoBasicStatement extends PBasicStatement
{
    private PAtribuicao _atribuicao_;
    private PSemicolon _semicolon_;

    public AAtribuicaoBasicStatement()
    {
        // Constructor
    }

    public AAtribuicaoBasicStatement(
        @SuppressWarnings("hiding") PAtribuicao _atribuicao_,
        @SuppressWarnings("hiding") PSemicolon _semicolon_)
    {
        // Constructor
        setAtribuicao(_atribuicao_);

        setSemicolon(_semicolon_);

    }

    @Override
    public Object clone()
    {
        return new AAtribuicaoBasicStatement(
            cloneNode(this._atribuicao_),
            cloneNode(this._semicolon_));
    }

    @Override
    public void apply(Switch sw)
    {
        ((Analysis) sw).caseAAtribuicaoBasicStatement(this);
    }

    public PAtribuicao getAtribuicao()
    {
        return this._atribuicao_;
    }

    public void setAtribuicao(PAtribuicao node)
    {
        if(this._atribuicao_ != null)
        {
            this._atribuicao_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._atribuicao_ = node;
    }

    public PSemicolon getSemicolon()
    {
        return this._semicolon_;
    }

    public void setSemicolon(PSemicolon node)
    {
        if(this._semicolon_ != null)
        {
            this._semicolon_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._semicolon_ = node;
    }

    @Override
    public String toString()
    {
        return ""
            + toString(this._atribuicao_)
            + toString(this._semicolon_);
    }

    @Override
    void removeChild(@SuppressWarnings("unused") Node child)
    {
        // Remove child
        if(this._atribuicao_ == child)
        {
            this._atribuicao_ = null;
            return;
        }

        if(this._semicolon_ == child)
        {
            this._semicolon_ = null;
            return;
        }

        throw new RuntimeException("Not a child.");
    }

    @Override
    void replaceChild(@SuppressWarnings("unused") Node oldChild, @SuppressWarnings("unused") Node newChild)
    {
        // Replace child
        if(this._atribuicao_ == oldChild)
        {
            setAtribuicao((PAtribuicao) newChild);
            return;
        }

        if(this._semicolon_ == oldChild)
        {
            setSemicolon((PSemicolon) newChild);
            return;
        }

        throw new RuntimeException("Not a child.");
    }
}
